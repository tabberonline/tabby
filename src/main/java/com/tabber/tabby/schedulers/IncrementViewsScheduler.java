    ...
    public void incrementViewsScheduler() {
        try {
            ...
            for(Tuple tuple : set) {
                ...
                Integer currentViews = userEntity.getPortfolio().getViews();
                portfolioEntity.setViews(currentViews==null ? 0 : currentViews + Double.valueOf(tuple.getScore()).intValue());
                portfolioRepository.saveAndFlush(portfolioEntity);
                userEntity.setPortfolio(portfolioEntity);
//                userEntity = userEntity.setResumePresent(userEntity);
